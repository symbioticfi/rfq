...
const targetPath = resolve(packageDir, "src", "generated", "deployment.json");
...
const deploymentEnv = normalizeDeploymentEnv(
  process.env.VITE_DEPLOYMENT_ENV || process.env.RFQ_DEPLOYMENT_ENV || fileEnv.VITE_DEPLOYMENT_ENV || fileEnv.RFQ_DEPLOYMENT_ENV || "local",
);
...
await writeFile(targetPath, `${JSON.stringify(normalizedManifest)}\n`);

// eslint-disable-next-line no-console
console.log(`Synced frontend deployment ${deploymentEnv} -> rfq/frontend/src/generated/deployment.json`);
