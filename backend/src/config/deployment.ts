...
const deploymentManifestSchema = z.object({
  ...
  contracts: z.object({
    ...
    vaultFactory: nullableAddressSchema,
  }),
...
/**
 * @dev Loads and validates the protocol deployment manifest.
 * @returns Parsed deployment manifest.
 */
export function loadProtocolDeploymentManifest(): DeploymentManifest {
  ...
  if (
    !parsed.contracts.permit2 ||
    !parsed.contracts.curatorRegistry ||
    !parsed.contracts.instantRedemptionAdapter ||
    !parsed.contracts.reactor ||
    parsed.vaults.length === 0
  ) {
    throw new Error("Generated deployment manifest is missing required protocol addresses");
  }
